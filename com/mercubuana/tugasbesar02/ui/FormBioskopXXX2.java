...
public class FormBioskopXXX {
	...
	//	}else {
			//	listDaftarFilm.getModel().setListData(null);
		//	}
	}
	...
	void hapusData() {
		...
		Film FilmDipilihPadaList = (Film) listDaftarFilm
				.getModel().getElementAt(index);
		//2. Meminta User memberikan Konfirmasi
		
		// 3. Hapus data dari database
		PersistentTransaction t = null;
		int indeksDataDipilih = listDaftarFilm
				.getSelectedIndex();
		int jumlahFilm= dataFilm.length;
		if (indeksDataDipilih==jumlahFilm-1&&
				jumlahFilm >=2) {
			/*yang dipilih adalah yang terahir dalam list
			*dan masih ada sisa Film yang belum dihapus
			*di dalam List
			*/
			indeksDataDipilih -=1;
		} else if (indeksDataDipilih==0 &&
				jumlahFilm == 1) {
			/*yang dipilih adalah satu-satunya Film
			 * dalam list
			 */
			indeksDataDipilih = -1;
		} 
